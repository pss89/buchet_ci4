...
<?= $this->section('content') ?>
  <div class="text-center text-2xl font-bold text-gray-900 mb-5">회원가입 - 모바일</div>
  <form class="max-w-sm mx-auto" name="joinForm">
    <div class="mb-5">
    ...
    <div class="mb-5">
      ...
      <input type="text" id="password" name="password" class="shadow-xs bg-gray-50 border border-gray-300 text-gray-900 text-sm 
        rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required />
    </div>
    <div class="mb-5">
      <label for="repeat-password" class="block mb-2 text-sm font-medium text-gray-900">비밀번호 확인</label>
      <input type="text" id="repeat-password" name="repeat-password" class="shadow-xs bg-gray-50 border border-gray-300 text-gray-900 text-sm 
        rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required />
    </div>
    <div class="mb-5">
...
<?= $this->section('scripts') ?>
  <script>
    function join() {
      // form 직렬화
      const formData = new FormData(document.forms['joinForm']);
